...
function getTime() {
  ...
  var tzHours = Math.floor(Math.abs(tz) / 60);
  var tzMinutes = Math.abs(tz) % 60
  var tzPrefix = (tz <= 0 ? '+' : '-')
  return [ time.getFullYear(), pad(time.getMonth() + 1), pad(time.getDay()) ].join('-') + 'T' + [ pad(time.getHours()), pad(time.getMinutes()), pad(time.getSeconds()) ].join(':') + '.' + pad(time.getMilliseconds(), 3) + tzPrefix + pad(tzHours) + ':' + pad(tzMinutes) + ' '
  function pad(num, pad) {
    return ('0000' + num).slice(pad ? (-1 * pad) : -2)
  }
}
function startDaemon(argv) {
  ...
  var dateInLog = JSON.parse(argv.shift());
  ...
  function createLogs(cb) {
    ...
    pend.wait(function(err) {
      ...
      if(dateInLog && naughtLog) {
        var write = naughtLog.write
        naughtLog.write = function(str) {
          str = getTime() + str
          write.apply(naughtLog, arguments);
        }
      }
      cb();
    });
